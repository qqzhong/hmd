...
wma_wow_get_pkt_proto_subtype(uint8_t *data, uint32_t len)
{
	...
	wma_info("Ether Type: 0x%04x, len=%d", eth_type, len);
	switch (eth_type) {
	...
	case QDF_NBUF_TRAC_IPV4_ETH_TYPE:
		...
		wma_info("IPV4 Packet");
		wma_log_pkt_ipv4(data, len);
